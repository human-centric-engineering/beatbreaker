...
const SHAPES: Array<[string, Pattern]> = [
  [
    'funk 4/4 x4',
    generatePattern({ style: 'funk', meter: '4/4', seed: 11, bars: 4, density: 60, ghosts: 60 }),
  ],
  [
    'funk 6/8 x2',
    generatePattern({ style: 'funk', meter: '6/8', seed: 12, bars: 2, density: 50, ghosts: 50 }),
  ],
  [
    'funk 7/8 x2',
    generatePattern({ style: 'funk', meter: '7/8', seed: 13, bars: 2, density: 50, ghosts: 50 }),
  ],
  [
    'funk 4/4 x1',
    generatePattern({ style: 'funk', meter: '4/4', seed: 14, bars: 1, density: 50, ghosts: 50 }),
  ],
  [
    'swing 4/4 x2',
    generatePattern({ style: 'swing', meter: '4/4', seed: 15, bars: 2, density: 50, ghosts: 50 }),
  ],
  [
    'bossa 4/4 x2',
    generatePattern({ style: 'bossa', meter: '4/4', seed: 16, bars: 2, density: 50, ghosts: 50 }),
  ],
  ...LIBRARY.slice(0, 6).map((item, i): [string, Pattern] => [
    item.title,
    patternFromLibrary(item, i),
  ]),
];
...
describe('doctor', () => {
  ...
  it.each(MOVES)(
    ...
    (move) => {
      for (const [name, pat] of SHAPES) {
        const before = JSON.stringify(pat);
        const out = doctor(pat, move, 7);
  ...
  it.each(MOVES)('%s is reproducible for a fixed entropy', (move) => {
    const [, pat] = SHAPES[0];
    expect(doctor(pat, move, 42)).toEqual(doctor(pat, move, 42));
  });

  const [, funk] = SHAPES[0];

  it('ghosts- removes every ghost and nothing else', () => {
    const out = doctor(funk, 'ghosts-', 1);
    out.bars.forEach((bar, bi) => {
  ...
  it('rejects nothing silently: an unknown move returns an unchanged copy', () => {
    const out = doctor(funk, 'nonsense' as DoctorMove, 1);
