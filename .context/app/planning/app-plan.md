...
| 4.6  | **Practice shelves** — a `Pin` table replacing `Break.pinned`: pin your own patterns _and_ library entries to **Practising** or **Later**; `/api/v1/pins` (enriched list, create, move, reorder, delete); ★ on library rows and on the stage                                                                                          | Route tests: pin a library entry and a pattern to each shelf, move between shelves, reorder; a pin on someone else's pattern vanishes from the list when it is unshared; erasure removes pins, export lists them; one request fills both shelves |
| 4.7  | **Practice history** — a `PracticeVisit` table replacing `Break.lastOpenedAt`: every pattern or library entry you open is recorded with the layer and tempo you left it at; `/api/v1/history` (enriched list, record, clear); a **Back** control and `Alt+←` / `Alt+→` to step through it; the list at the top of the Patterns drawer | Route tests: a second visit moves an item to the top rather than duplicating it; the list is capped at 50; clear empties it; the Studio test toggles to the previous item and lands on its layer and tempo; erasure and export as above          |
...
- **Shelves.** Pin things you are actively practising, and things you like and
  ...
  not only your own patterns. A boolean on `Break` cannot do either half: it has
...
- **History.** Remember what you were recently learning, and let you toggle
  back to it quickly. `Break.lastOpenedAt` records that a _saved pattern_ was
  opened; it cannot hold a famous break, and it does not know where you were in
  it. A visit row does: `PracticeVisit { userId, breakId? | libraryEntryId?,
level, bpm, visitedAt }`, one per target (a revisit moves it to the top), the
  newest 50 kept. The point of storing the layer and tempo is the toggle: going
...
Both supersede columns tasks 4.1–4.3 added (`pinned`, `lastOpenedAt`, the
`pinned` filter and `sort=opened`). None of that has been merged or deployed, so
**the recommendation is to remove them in the same PR rather than keep two
records of one fact** (D17, D18). Both new tables are personal data: `userId`
cascades, and each gets an export section.
...
| 4     | New `Pin` — `userId` (cascade), `shelf` (`practising`/`later`), `breakId?` → `Break` (cascade), `libraryEntryId?` → `LibraryEntry` (cascade), `position Int`, `createdAt`; CHECK exactly one target; unique `(userId, breakId)` and `(userId, libraryEntryId)`; replaces `Break.pinned` (D17).                                           | cascade                                                  | new section `pins`                              |
| 4     | New `PracticeVisit` — `userId` (cascade), `breakId?` (cascade), `libraryEntryId?` (cascade), `level Int`, `bpm Int`, `visitedAt`; CHECK exactly one target; unique per `(userId, target)`; newest 50 kept per user; replaces `Break.lastOpenedAt` (D18).                                                                                 | cascade                                                  | new section `practiceHistory`                   |
...
| D17 | Shelves: two, and do they replace `Break.pinned`?                       | **Two — Practising and Later — in a `Pin` table that also holds library entries, replacing `Break.pinned` before Phase 4 merges.** One shelf cannot say "want to learn"; a column cannot pin a famous break.                                          | task 4.6  |
| D18 | History: does it replace `Break.lastOpenedAt`, and how much is kept?    | **Replace it, keep the newest 50, store layer and tempo per visit.** One record of "what I opened", not two that drift. 50 is weeks of daily practice; older visits fall off silently.                                                                | task 4.7  |
